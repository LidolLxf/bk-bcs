...
// Package bcsthirdpartyservice xxx
...
// SetClientConfig set bcs thirdparty service  client config
...
func GetClient(innerClientName string) (BcsThirdpartyServiceClient, func(), error) {
	if clientConfig == nil {
	...
	if discovery.UseServiceDiscovery() {
		...
		if clientConfig.Discovery == nil {
			return nil, nil, fmt.Errorf("thirdparty service module not enable discovery")
		}
	...
	klog.Infof("get thirdparty service client with address: %s", addr)
	conf := &bcsapi.Config{
		Hosts:           []string{addr},
		TLSConfig:       clientConfig.TLSConfig,
		InnerClientName: innerClientName,
	}
	cli, closeCon, err := NewThirdpartyServiceClient(conf)
	if err != nil {
		return nil, nil, err
	}
...
// newThirdpartyServiceConn creates a ThirdpartyService grpc connection
func newThirdpartyServiceConn(config *bcsapi.Config) (*grpc.ClientConn, func(), error) {
	...
	for i := 0; i < maxTries; i++ {
		...
		if err != nil {
			klog.Errorf("Create thirdparty service grpc client with %s error: %s", addr, err.Error())
			continue
		}
		if conn != nil {
			break
	...
	if conn == nil {
		klog.Errorf("create no thirdparty service client after all instance tries")
		return nil, nil, fmt.Errorf("failed to create grpc connection")
	}
	return conn, func() { _ = conn.Close() }, nil
}

// NewThirdpartyServiceClient create ThirdpartyService SDK implementation
func NewThirdpartyServiceClient(config *bcsapi.Config) (BcsThirdpartyServiceClient, func(), error) {
	conn, closeFn, err := newThirdpartyServiceConn(config)
	if err != nil {
		return nil, nil, err
	}
	return NewBcsThirdpartyServiceClient(conn), closeFn, nil
}
