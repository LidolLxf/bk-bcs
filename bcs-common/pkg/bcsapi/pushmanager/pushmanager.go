...
func GetClient(innerClientName string) (PushManagerClient, func(), error) {
	...
	cli, closeCon, err := NewPushClient(conf)
	if err != nil {
		return nil, nil, err
	}
...
// newPushConn creates a grpc connection for push manager clients
func newPushConn(config *bcsapi.Config) (*grpc.ClientConn, func(), error) {
...
// NewPushClient create PushManager SDK implementation
func NewPushClient(config *bcsapi.Config) (PushManagerClient, func(), error) {
	conn, closeFn, err := newPushConn(config)
	if err != nil {
		return nil, nil, err
	}
	return NewPushManagerClient(conn), closeFn, nil
}
