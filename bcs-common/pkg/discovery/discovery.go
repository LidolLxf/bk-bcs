		...
		for {
			...
			default:
				if watcher == nil {
					watcher, err = md.microRegistry.Watch(registry.WatchService(md.module))
					if err != nil {
						klog.Error("see empty watcher and failed to create registry watcher for module %s, err %s",
							md.module, err.Error())
				...
				if err := md.watchRegistry(watcher); err != nil {
					klog.Error("failed when watching registry for module %s, err %s", md.module, err.Error())
					time.Sleep(rewatchSecond * time.Second)
				}
...
func (md *ModuleDiscovery) watchRegistry(w registry.Watcher) error {
	...
	if err != nil {
		klog.Error("failed to get service for module %s, err %s", md.module, err.Error())
		return err
	}
	md.Lock()
	md.curServices = svcs
	...
	for {
		...
		if result != nil && result.Service != nil {
			klog.Info("service watch result, action %s, service %s", result.Action, result.Service.Name)
		}
		// receive event, to update service
		svcs, err := md.microRegistry.GetService(md.module)
		if err != nil {
...
func (md *ModuleDiscovery) GetRandomServiceNode() (*registry.Node, error) {
	...
	if len(md.curServices) == 0 {
		klog.Info("discovery has no local service cache[%s]", md.module)
	...
	if nodeLength == 0 {
		klog.Info("discovery found no node information of %s", md.module)
