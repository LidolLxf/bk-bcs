...
func (g *general) getProCondition() *operator.Condition {
	features := operator.M{constants.ProjectIDTag: g.projectID}
	return operator.NewLeafCondition(operator.Eq, features)
}

func (g *general) putResources() error {
	return project.PutData(g.ctx, g.data, g.resourceFeatList, g.getProCondition())
}
