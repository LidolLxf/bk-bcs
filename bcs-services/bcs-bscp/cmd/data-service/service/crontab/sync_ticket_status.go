	...
	for _, ticket := range tickets {
		...
		if ticket.CurrentStatus == constant.TicketRunningStatu {
			...
			if len(ticketStatus.Data.CurrentSteps) == 0 {
				gan, err := itsm.GetApproveNodeResult(kt.Ctx, ticket.SN, strategyMap[ticket.SN].Spec.ItsmTicketStateID)
				if err != nil {
					return
				}
				req := &pbds.ApproveReq{
					...
					PublishStatus: string(table.RejectedApproval),
				...
				ctx := metadata.NewIncomingContext(kt.Ctx, metadata.MD{
					strings.ToLower(constant.UserKey): []string{gan.Data.Processeduser},
				})
