...
func GetWorkflowByService(serviceID int) (int, error) {
...
func GetStateApproveByWorkfolw(workflowID int) (int, error) {
	itsmConf := cc.DataService().ITSM
	// 默认使用网关访问，如果为外部版，则使用ESB访问
	host := itsmConf.GatewayHost
	if itsmConf.External {
...
func UpdateService(data UpdateServiceReq) error {
	...
	// 请求API
	body, err := ItsmRequest(kt.Ctx, http.MethodPost, reqURL, reqData)
	if err != nil {
