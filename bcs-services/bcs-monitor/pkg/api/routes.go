...
func registerRoutes() http.Handler {
	...
	r.Route("/projects/{projectId}/clusters/{clusterId}", func(route chi.Router) {
		...
		// bk-log 日志采集规则
		route.Post("/log_collector/entrypoints", rest.Handler(logrule.GetEntrypoints))
		route.Get("/log_collector/rules", rest.Handler(logrule.ListLogCollectors))
		route.Post("/log_collector/rules", rest.Handler(logrule.CreateLogRule))
		route.Get("/log_collector/rules/{id}", rest.Handler(logrule.GetLogRule))
		route.Put("/log_collector/rules/{id}", rest.Handler(logrule.UpdateLogRule))
		route.Delete("/log_collector/rules/{id}", rest.Handler(logrule.DeleteLogRule))
		route.Post("/log_collector/rules/{id}/retry", rest.Handler(logrule.RetryLogRule))
		route.Post("/log_collector/rules/{id}/enable", rest.Handler(logrule.EnableLogRule))
		route.Post("/log_collector/rules/{id}/disable", rest.Handler(logrule.DisableLogRule))
		route.Get("/log_collector/storages/cluster_groups", rest.Handler(logrule.GetStorageClusters))
		route.Post("/log_collector/storages/switch_storage", rest.Handler(logrule.SwitchStorage))
	})
	return r
}
...
func registerMetricsRoutes() http.Handler {
	...
	r.Route("/projects/{projectCode}/clusters/{clusterId}", func(route chi.Router) {
		...
		route.Get("/overview", rest.Handler(metrics.GetClusterOverview))
		route.Get("/cpu_usage", rest.Handler(metrics.ClusterCPUUsage))
		route.Get("/cpu_request_usage", rest.Handler(metrics.ClusterCPURequestUsage))
		route.Get("/memory_usage", rest.Handler(metrics.ClusterMemoryUsage))
		route.Get("/memory_request_usage", rest.Handler(metrics.ClusterMemoryRequestUsage))
		route.Get("/disk_usage", rest.Handler(metrics.ClusterDiskUsage))
		route.Get("/diskio_usage", rest.Handler(metrics.ClusterDiskioUsage))
		route.Get("/pod_usage", rest.Handler(metrics.ClusterPodUsage))
		route.Get("/nodegroup/{nodegroup}/node_num", rest.Handler(metrics.ClusterGroupNodeNum))
		route.Get("/nodegroup/{nodegroup}/max_node_num", rest.Handler(metrics.ClusterGroupMaxNodeNum))
		route.Get("/nodes/{node}/info", rest.Handler(metrics.GetNodeInfo))
		route.Get("/nodes/{node}/overview", rest.Handler(metrics.GetNodeOverview))
		route.Post("/nodes/overviews", rest.Handler(metrics.ListNodeOverviews))
		route.Get("/nodes/{node}/cpu_usage", rest.Handler(metrics.GetNodeCPUUsage))
		route.Get("/nodes/{node}/cpu_request_usage", rest.Handler(metrics.GetNodeCPURequestUsage))
		route.Get("/nodes/{node}/memory_usage", rest.Handler(metrics.GetNodeMemoryUsage))
		route.Get("/nodes/{node}/memory_request_usage", rest.Handler(metrics.GetNodeMemoryRequestUsage))
		route.Get("/nodes/{node}/network_receive", rest.Handler(metrics.GetNodeNetworkReceiveUsage))
		route.Get("/nodes/{node}/network_transmit", rest.Handler(metrics.GetNodeNetworkTransmitUsage))
		route.Get("/nodes/{node}/disk_usage", rest.Handler(metrics.GetNodeDiskUsage))
		route.Get("/nodes/{node}/diskio_usage", rest.Handler(metrics.GetNodeDiskioUsage))
		route.Post("/namespaces/{namespace}/pods/cpu_usage", rest.Handler(
			metrics.PodCPUUsage)) // 多个Pod场景, 可能有几十，上百Pod场景, 需要使用 Post 传递参数
		route.Post("/namespaces/{namespace}/pods/cpu_limit_usage", rest.Handler(metrics.PodCPULimitUsage))
		route.Post("/namespaces/{namespace}/pods/cpu_request_usage", rest.Handler(metrics.PodCPURequestUsage))
		route.Post("/namespaces/{namespace}/pods/memory_used", rest.Handler(metrics.PodMemoryUsed))
		route.Post("/namespaces/{namespace}/pods/network_receive", rest.Handler(metrics.PodNetworkReceive))
		route.Post("/namespaces/{namespace}/pods/network_transmit", rest.Handler(metrics.PodNetworkTransmit))
		route.Get("/namespaces/{namespace}/pods/{pod}/containers/{container}/cpu_usage",
			rest.Handler(metrics.ContainerCPUUsage))
		route.Get("/namespaces/{namespace}/pods/{pod}/containers/{container}/memory_used",
			rest.Handler(metrics.ContainerMemoryUsed))
		route.Get("/namespaces/{namespace}/pods/{pod}/containers/{container}/cpu_limit",
			rest.Handler(metrics.ContainerCPULimit))
		route.Get("/namespaces/{namespace}/pods/{pod}/containers/{container}/memory_limit",
			rest.Handler(metrics.ContainerMemoryLimit))
		route.Get("/namespaces/{namespace}/pods/{pod}/containers/{container}/disk_read_total",
			rest.Handler(metrics.ContainerDiskReadTotal))
		route.Get("/namespaces/{namespace}/pods/{pod}/containers/{container}/disk_write_total",
			rest.Handler(metrics.ContainerDiskWriteTotal))

		route.Get("/namespaces/{namespace}/service_monitors",
			rest.Handler(service_monitor.ListServiceMonitors))
		route.Get("/namespaces/{namespace}/service_monitors/{name}",
			rest.Handler(service_monitor.GetServiceMonitor))
		route.Post("/namespaces/{namespace}/service_monitors",
			rest.Handler(service_monitor.CreateServiceMonitor))
		route.Put("/namespaces/{namespace}/service_monitors/{name}",
			rest.Handler(service_monitor.UpdateServiceMonitor))
		route.Delete("/namespaces/{namespace}/service_monitors/{name}",
			rest.Handler(service_monitor.DeleteServiceMonitor))
		route.Get("/service_monitors",
			rest.Handler(service_monitor.ListServiceMonitors))
		route.Post("/service_monitors/batchdelete",
			rest.Handler(service_monitor.BatchDeleteServiceMonitor))

		route.Get("/namespaces/{namespace}/pod_monitors",
			rest.Handler(podmonitor.ListPodMonitors))
		route.Get("/namespaces/{namespace}/pod_monitors/{name}",
			rest.Handler(podmonitor.GetPodMonitor))
		route.Post("/namespaces/{namespace}/pod_monitors",
			rest.Handler(podmonitor.CreatePodMonitor))
		route.Put("/namespaces/{namespace}/pod_monitors/{name}",
			rest.Handler(podmonitor.UpdatePodMonitor))
		route.Delete("/namespaces/{namespace}/pod_monitors/{name}",
			rest.Handler(podmonitor.DeletePodMonitor))
		route.Get("/pod_monitors",
			rest.Handler(podmonitor.ListPodMonitors))
		route.Post("/pod_monitors/batchdelete",
			rest.Handler(podmonitor.BatchDeletePodMonitor))
